...
packages            (id, trainer_id, client_id, total_sessions, used_sessions,
                     price_per_session)                              -- V13: nullable, per-client price
...
nutrition_plans     (id, trainer_id, client_id|managed_client_id, targets jsonb,
                     plan_markdown, created_at)                      -- V15
```

Derived (SQL views, not tables): `client_streaks`, `client_prs`, `sessions_remaining`,
`trainer_monthly_money` (V13), `client_risk` (V16).
...
**V12 — Hebrew + RTL + Intake + Leaderboard.** Full i18n (en/he) with RTL flip via `I18nManager`. Intake questionnaire on client signup (goals, injuries, available equipment, experience). Opt-in weekly group leaderboard (most workouts / longest streak) for group clients. *(Built as V12a + V12b; the leaderboard was dropped from V12b's scope by decision — moved to §8.)*

### 5.1 Post-1.0 roadmap (V13–V16)
...
**V14b — Device Calendar Sync (bonus).** Opt-in via a Profile toggle, strictly **one-way**: creating/moving/deleting a scheduled workout mirrors an event into a dedicated "CoachFlow" calendar on the device via `expo-calendar` (covers iPhone Calendar and any Google account synced to the phone). The app stays the source of truth — events edited in the device calendar are never read back. Event/calendar ids are kept locally (AsyncStorage); nothing new in the DB.

**V15 — AI Nutrition Assistant.** The trainer enters a client's daily targets — calories, protein/carbs/fat grams, optional preferences (vegetarian, kosher, allergies, number of meals) — and gets back a one-day meal plan with per-meal macro estimates, in the app language, generated by the Claude API. The Anthropic API key lives **only** as a Supabase Edge Function secret (`nutrition-suggest`, same server-side pattern as V11's reminders) — never in the app bundle. The latest saved plan per client is stored in `nutrition_plans` (trainer full access, client reads own) so the client can see it read-only. Every response carries a fixed "general suggestions — not medical or dietetic advice" disclaimer, consistent with the V1 health disclaimer. *(The trainer isn't a nutritionist — this gives clients real guidance without pretending to be one.)*

**V16 — Client Retention Radar.** A deterministic `security_invoker` SQL view (`client_risk`) flags at-risk clients per trainer with a reason: `missed_streak` (≥2 consecutive derived-missed scheduled workouts) or `gone_quiet` (no completed workout in the last 10 days despite at least one in the 30 days before that). No stored flags, no cron, no AI — derived on read exactly like streaks, dates in Asia/Jerusalem. The trainer Home gains an "at-risk" card listing flagged clients with a reason chip; tapping opens the client, and the V11 WhatsApp deep link gives a one-tap nudge. *(Retention is the trainer's revenue — losing a client silently is the most expensive bug.)*
...
| AI nutrition assistant (macros → meal plan) | V15 |
| Client retention radar (at-risk alerts) | V16 |

---

## 8. Out of Scope / Future
Deferred by design — none of these block launch (V13–V16 above are now specified in §5.1; these are what remains beyond them):
- Group leaderboard (dropped from V12b's scope by decision; revisit only if group clients ask for it).
- Native push notifications (Expo Notifications + EAS) — easy to add later; would pair naturally with V14's schedule focus.
